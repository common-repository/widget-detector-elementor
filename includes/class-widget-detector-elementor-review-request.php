 ...
 *
 * TODO Search & Replace widget_detector_elementor_ with your widget_detector_elementor
 * TODO Search & Replace widget_detector_elementor_ with your widget_detector_elementor
 * TODO Search & Replace 'w-d-e' with your 'w-d-e'
...
/**
 * Class Widget_Detector_Elementor_Modules_Reviews
 *
...
class Widget_Detector_Elementor_Modules_Reviews {
	...
	public static function init() {
		...
		add_action( 'wp_ajax_widget_detector_elementor_review_action', array( __CLASS__, 'ajax_handler' ) );
	}
	...
	public static function installed_on() {
		$installed_on = get_option( 'widget_detector_elementor_reviews_installed_on', false );

		if ( ! $installed_on ) {
			$installed_on = current_time( 'mysql' );
			update_option( 'widget_detector_elementor_reviews_installed_on', $installed_on );
		}
	...
	public static function ajax_handler() {
		...
		if ( ! wp_verify_nonce( $_REQUEST['nonce'], 'widget_detector_elementor_review_action' ) ) {
			wp_send_json_error();
		}
		...
		try {
			...
			update_user_meta( $user_id, '_widget_detector_elementor_reviews_dismissed_triggers', $dismissed_triggers );
			update_user_meta( $user_id, '_widget_detector_elementor_reviews_last_dismissed', current_time( 'mysql' ) );
	...
	public static function dismissed_triggers() {
		...
		$dismissed_triggers = get_user_meta( $user_id, '_widget_detector_elementor_reviews_dismissed_triggers', true );
	...
	public static function already_did( $set = false ) {
		...
		if ( $set ) {
			update_user_meta( $user_id, '_widget_detector_elementor_reviews_already_did', true );

			return true;
		}

		return (bool) get_user_meta( $user_id, '_widget_detector_elementor_reviews_already_did', true );
	}
	...
	public static function triggers( $group = null, $code = null ) {
		...
		if ( ! isset( $triggers ) ) {
			...
			$open_message = __( 'Hey, I noticed you recently hit %d popup views on your site – that’s awesome!. I would very much appreciate if you could quickly give it a 5-star rating on WordPress, just to help us spread the word.', 'w-d-e' );
			$time_message = __( "Hey, you've been using Widget Detector for Elementor for %s on your site - I hope that its been helpful. I would very much appreciate if you could quickly give it a 5-star rating on WordPress, just to help us spread the word and motivate us to make it even better.", 'w-d-e' );

			$triggers = apply_filters( 'widget_detector_elementor_reviews_triggers', array(
				'time_installed' => array(
					'triggers' => array(
						'one_week'     => array(
							'message'    => sprintf( $time_message, __( '1 week', 'w-d-e' ) ),
							'conditions' => array(
								strtotime( self::installed_on() . ' +1 week' ) < time(),
							),
							'link'       => 'https://wordpress.org/support/plugin/widget-detector-elementor/reviews/?rate=5#rate-response',
							'pri'        => 10,
						),
						'one_month'    => array(
							'message'    => sprintf( $time_message, __( '1 month', 'w-d-e' ) ),
							'conditions' => array(
						...
						'three_months' => array(
							'message'    => sprintf( $time_message, __( '3 months', 'w-d-e' ) ),
							'conditions' => array(
				...
				'open_count'     => array(
					'triggers' => array(
						'50_opens'  => array(
							...
							'conditions' => array(
								get_option( 'widget_detector_elementor_total_open_count', 0 ) > 50,
							),
							'link'       => 'https://wordpress.org/support/plugin/widget-detector-elementor/reviews/?rate=5#rate-response',
							'pri'        => 10,
						),
						'100_opens' => array(
							...
							'conditions' => array(
								get_option( 'widget_detector_elementor_total_open_count', 0 ) > 100,
						...
						'500_opens' => array(
							...
							'conditions' => array(
								get_option( 'widget_detector_elementor_total_open_count', 0 ) > 500,
							),
							'link'       => 'https://wordpress.org/support/plugin/widget-detector-elementor/reviews/?rate=5#rate-response',
							'pri'        => 30,
						),
	...
	public static function admin_notices() {
			...
			(function ($) {
				...
				function dismiss(reason) {
					$.ajax({
						...
						url: ajaxurl,
						data: {
							action: 'widget_detector_elementor_review_action',
							nonce: '<?php echo wp_create_nonce( 'widget_detector_elementor_review_action' ); ?>',
							group: trigger.group,
							code: trigger.code,
							pri: trigger.pri,
							reason: reason
				...
				$(document)
					.on('click', '.widget_detector_elementor-notice .widget_detector_elementor-dismiss', function (event) {
						var $this = $(this),
							reason = $this.data('reason'),
							notice = $this.parents('.widget_detector_elementor-notice');
					...
					.ready(function () {
						setTimeout(function () {
							$('.widget_detector_elementor-notice button.notice-dismiss').click(function (event) {
								dismiss('maybe_later');
							});
						}, 1000);
					});
		...
		<style>
			.widget_detector_elementor-notice p {
				margin-bottom: 0;
			}

			.widget_detector_elementor-notice img.logo {
				float: right;
				margin-left: 10px;
				width: 75px;
				padding: 0.25em;
		...
		<div class="notice notice-success is-dismissible widget_detector_elementor-notice">

			<p>
				<img class="logo" src="<?php echo WIDGET_DETECTOR_ELEMENTOR_URL; ?>admin/images/icon-256x256.png" />
				<strong>
			...
			<ul>
				<li>
					<a class="widget_detector_elementor-dismiss" target="_blank" href="https://wordpress.org/support/plugin/widget-detector-elementor/reviews/?rate=5#rate-response" data-reason="am_now">
						<strong><?php _e( 'Ok, I will help you with 5 star rating', 'w-d-e' ); ?></strong>
					</a>
				</li>
				<li>
					<a href="https://elementinvader.com/faq#ticket" target="_blank"class="widget_detector_elementor-dismiss" data-reason="improve_suggestion">
						<?php _e( 'No, you doesn\'t deserver 5 stars and I will tell you why!', 'w-d-e' ); ?>
					</a>
				</li>
				<li>
					<a href="#" class="widget_detector_elementor-dismiss" data-reason="maybe_later">
						<?php _e( 'Nope, maybe later', 'w-d-e' ); ?>
					</a>
				</li>
				<li>
					<a href="#" class="widget_detector_elementor-dismiss" data-reason="already_did">
						<?php _e( 'I already did', 'w-d-e' ); ?>
					</a>
				</li>
			</ul>
	...
	public static function last_dismissed() {
		...
		return get_user_meta( $user_id, '_widget_detector_elementor_reviews_last_dismissed', true );
	}
...
Widget_Detector_Elementor_Modules_Reviews::init();
